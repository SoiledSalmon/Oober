"""
Runs end-to-end pipeline and constraint validation checks.

This module is part of the Oober joint price-and-match
optimisation system. It runs a multi-window simulation trace and asserts
all optimization constraints (matching uniqueness, price bounds, stability, fairness).
"""
...
def run_e2e_pipeline() -> bool:
    """Runs a 5-window simulation and validates all optimization constraints.

    Returns:
        bool: True if all end-to-end constraints are successfully validated.

    Raises:
        AssertionError: If any of the optimization constraints are violated.
    """
    ...
    for window in trace_data["windows"]:
        ...
        print(
            f"\n--- Window {window_id} (Status: {status}, "
            f"Matches: {len(assignments)}) ---"
        )
        ...
        # Calculate target midpoint earnings target_midpoint for this window
        feasible_midpoints = []
        for r in riders:
            for d in drivers:
                if r["wtp"] >= d["maf"]:
        ...
        target_midpoint = (
            np.mean(feasible_midpoints) if feasible_midpoints else 0.0
        )
        ...
        for rid, did, price in assignments:
            ...
            assert rid not in assigned_riders, (
                f"Rider {rid} matched multiple times!"
            )
            assigned_riders.add(rid)

            # Constraint 2: Driver Uniqueness
            assert did not in assigned_drivers, (
                f"Driver {did} matched multiple times!"
            )
            assigned_drivers.add(did)

            # Constraint 3: Edge Feasibility
            assert price >= driver["maf"] - 1e-5, (
                f"Price {price} below driver MAF {driver['maf']}!"
            )
            assert price <= rider["wtp"] + 1e-5, (
                f"Price {price} above rider WTP {rider['wtp']}!"
            )
            ...
            if status in ["Optimal", "Feasible"]:
                if corridor in price_memory:
                    ...
                    assert diff <= max_diff, (
                        f"Price deviation error: price={price}, "
                        f"prev={prev_price}, diff={diff}, "
                        f"max_diff={max_diff}"
                    )
            ...
            if status in ["Optimal", "Feasible"]:
                assert (
                    price <= target_midpoint * (1.0 + fairness_tolerance)
                    + 0.01
                    + 1e-5
                ), f"Driver earnings {price} above fairness bound!"
                assert (
                    price >= target_midpoint * (1.0 - fairness_tolerance)
                    - 0.01
                    - 1e-5
                ), f"Driver earnings {price} below fairness bound!"
