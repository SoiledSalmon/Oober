...
class TestBackendAPI(unittest.TestCase):
    ...
    def test_simulate_valid_parameters(self):
        """Verify that /api/simulate returns 200 OK and valid trace JSON for default/valid inputs."""
        payload = {
            "num_windows": 5,
            "delta": 0.10,
    ...
    def test_simulate_boundary_parameters(self):
        """Verify that /api/simulate allows boundary values matching backend pydantic limits."""
        payloads = [
            # Minimum boundaries
            {"num_windows": 1, "delta": 0.01, "fairness_tolerance": 0.01, "num_zones": 3, "seed": 0},
            # Maximum boundaries
            {"num_windows": 50, "delta": 1.0, "fairness_tolerance": 1.0, "num_zones": 30, "seed": 9999}
        ]
    ...
    def test_simulate_invalid_num_windows(self):
    ...
    def test_simulate_invalid_fairness_tolerance(self):
        """Verify that fairness_tolerance bounds (0.01-1.0) are enforced, returning 422 JSON."""
        # Under limit
        response = self.client.post("/api/simulate", json={"fairness_tolerance": 0.009})
        self.assertEqual(response.status_code, 422)
        
        # Over limit
        response = self.client.post("/api/simulate", json={"fairness_tolerance": 1.01})
        self.assertEqual(response.status_code, 422)

    def test_simulate_invalid_num_zones(self):
    ...
    def test_simulate_invalid_seed(self):
