"""
Unit tests for the performance metrics computation in the Oober system.

This module is part of the Oober joint price-and-match
optimisation system. It exercises wait time, earnings variance,
price deviation, and matching rate formulas under standard and empty setups.
"""
...
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "oober"))
)
...
class TestMetrics(unittest.TestCase):
    """Test suite for verifying the logic of system optimization metrics."""

    def setUp(self) -> None:
        """Setup test bipartite feasibility graph and input lists before each test."""
        # 1. Create a dummy feasibility graph (bipartite)
        self.feas_graph = nx.Graph()

        # Add rider nodes (bipartite=0)
        self.feas_graph.add_node(
            ("rider", 0),
            bipartite=0,
            data={"id": 0, "origin_zone": 0, "dest_zone": 3, "wtp": 60},
        )
        self.feas_graph.add_node(
            ("rider", 1),
            bipartite=0,
            data={"id": 1, "origin_zone": 1, "dest_zone": 4, "wtp": 45},
        )
        self.feas_graph.add_node(
            ("rider", 2),
            bipartite=0,
            data={"id": 2, "origin_zone": 2, "dest_zone": 5, "wtp": 80},
        )
        ...
        self.feas_graph.add_node(
            ("driver", 0),
            bipartite=1,
            data={"id": 0, "current_zone": 0, "maf": 20},
        )
        self.feas_graph.add_node(
            ("driver", 1),
            bipartite=1,
            data={"id": 1, "current_zone": 1, "maf": 35},
        )
        self.feas_graph.add_node(
            ("driver", 2),
            bipartite=1,
            data={"id": 2, "current_zone": 2, "maf": 25},
        )

        # Add edges with attributes
        self.feas_graph.add_edge(
            ("rider", 0),
            ("driver", 0),
            travel_cost=10.0,
            price_lb=20.0,
    ...
    def test_compute_wait_time(self) -> None:
        """Verify compute_wait_time returns correct travel cost sum
        for assignments.
        """
    ...
    def test_compute_wait_time_empty(self) -> None:
        """Verify compute_wait_time returns 0.0 when assignments list is empty."""
        assignments = []
        try:
            wait_time = compute_wait_time(assignments, self.feas_graph)
    ...
    def test_compute_earnings_variance(self) -> None:
        """Verify compute_earnings_variance calculates variance over
        matched drivers correctly.
        """
        # Assignments:
        # Driver 0: earns 40.0
        # Driver 2: earns 70.0
    ...
    def test_compute_earnings_variance_empty(self) -> None:
        """Verify compute_earnings_variance returns 0.0 when assignments list is empty."""
        assignments = []
        # All drivers earn 0
        expected_var = 0.0
        try:
    ...
    def test_compute_price_deviation(self) -> None:
        """Verify compute_price_deviation computes the correct fraction
        of price limit violations.
        """
        # Price memory: corridor (0, 3) was 50.0, corridor (1, 4) was 30.0
        price_memory = {(0, 3): 50.0, (1, 4): 30.0}
        ...
        assignments = [(0, 0, 52.0), (1, 1, 40.0), (2, 2, 75.0)]

        try:
            deviation = compute_price_deviation(
                assignments, price_memory, self.riders, delta=0.10
            )
            self.assertAlmostEqual(deviation, 1.0 / 2.0)
        except NotImplementedError:
            self.skipTest("compute_price_deviation not implemented yet")

    def test_compute_price_deviation_empty_memory(self) -> None:
        """Verify compute_price_deviation returns 0.0 when price memory is empty."""
        price_memory = {}
        assignments = [(0, 0, 52.0), (1, 1, 40.0), (2, 2, 75.0)]
        try:
            deviation = compute_price_deviation(
    ...
    def test_compute_price_deviation_empty_assignments(self) -> None:
        """Verify compute_price_deviation returns 0.0 when assignments list is empty."""
    ...
    def test_compute_matching_rate(self) -> None:
        """Verify compute_matching_rate calculates correct ratio of
        matched to total riders.
        """
        assignments = [(0, 0, 40.0), (2, 2, 70.0)]
    ...
    def test_compute_matching_rate_zero_riders(self) -> None:
        """Verify compute_matching_rate returns 0.0 when total riders count is zero."""
        assignments = []
        try:
