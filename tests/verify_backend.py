"""
Verifies the backend by running unit tests and checking basic simulation runs.

This module is part of the Oober joint price-and-match
optimisation system. It serves as a sanity check script
to discover and execute the unit test suite and verify basic trace schema.
"""
...
def run_unit_tests() -> bool:
    """Discovers and runs the unittest test suite.

    Returns:
        bool: True if the test suite passed successfully, False otherwise.
    """
    ...
    print(
        f"Tests run: {result.testsRun}, Errors: {len(result.errors)}, "
        f"Failures: {len(result.failures)}"
    )
    return result.wasSuccessful()


def run_simulation_checks() -> bool:
    """Runs a 5-window simulation and verifies output schema correctness.

    Returns:
        bool: True if simulation schema checks pass, False otherwise.
    """
    ...
    try:
        ...
        assert len(graph["nodes"]) == 8, (
            f"Expected 8 nodes, got {len(graph['nodes'])}"
        )
        ...
        for idx, win in enumerate(windows):
            ...
            assert "joint_opt_assignments" in win, (
                f"Window {idx} missing joint_opt_assignments"
            )
            assert "seq_baseline_assignments" in win, (
                f"Window {idx} missing seq_baseline_assignments"
            )
            assert "joint_opt_wait_time" in win, (
                f"Window {idx} missing joint_opt_wait_time"
            )
            assert "seq_baseline_wait_time" in win, (
                f"Window {idx} missing seq_baseline_wait_time"
            )
